...
namespace FX.Mobile.DeveloperTools.Content
{
	public partial class StartMobileWebsiteControl : ActionPanel
	{
		...
		private void buttonBrowse_Click(object sender, EventArgs e)
		{
			using (var dlg = new FolderBrowserDialog())
			{
				...
				if (dlg.ShowDialog() == DialogResult.OK)
				{
					...
					panelWarning.Visible = (!(EnvironmentHasSDK() && EnvironmentHasSalesLogix()));
				}
			}
		}
		...
		private void buttonStart_Click(object sender, EventArgs e)
		{
			...
			process.StartInfo.FileName = GetLauncherPath();
			process.StartInfo.Arguments = "/path:\"" + textProductPath.Text + "\"";
			process.Start();
		...
		private bool EnvironmentHasSalesLogix()
		{
			return Directory.Exists(Path.Combine(textProductPath.Text, @"products\argos-saleslogix"));
		}
